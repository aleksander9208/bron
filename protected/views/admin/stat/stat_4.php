...
<div class="accordion" id="accordion_ur">
    <?php foreach ($statData['questionnaire'] as $name_ur =>  $k) { ?>
        <div class="card">
            <div class="card-header btn btn-link text-left" id="ur_header_<?php echo md5($name_ur); ?>" data-toggle="collapse" data-target="#ur_<?php echo md5($name_ur); ?>" aria-expanded="false" aria-controls="ur_<?php echo md5($name_ur); ?>">Сводная таблица по ЮЛ <b><?php echo CHtml::encode($name_ur); ?></b></div>
                ...
                <div class="card-body p-0">
                        ...
                        <?php foreach ($k as  $v) { ?>
                            ...
                            <div class="text-right">
                                <button class="btn btn-info btn-sm my-1 z_btn_print" role="button" data-target="#z_anketa_table_<?php echo md5($name_ur); ?>_<?php echo $v['id'];?>">Печать таблицы</button>
                            </div>
                            <table id="z_anketa_table_<?php echo md5($name_ur); ?>_<?php echo $v['id'];?>" class="table table-bordered table-striped table-hover table-sm">
                                ...
                                <?php }  ?>
                                    <tr class="<?php echo($v['create_admin']? 'table-info' : ''); ?>">
                                        <th scope="row" class="align-middle"><?php echo Questionnaire::getCAMPName($v['camp_id']); ?></th>
