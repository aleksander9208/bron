...
env: ## Создаёт .env .settings.php dbconn.php
	...
	@if [ ! -f ./public_html/protected/config/config-constants.php ]; then \
		cp ./public_html/protected/config/config-constants-example.php ./public_html/protected/config/config-constants.php; \
	fi
...
backup:
	docker exec -it database sh -c "mysqldump -uroot --password=$(MYSQL_ROOT_PASSWORD) --databases $(MYSQL_DATABASE) | gzip > /docker-entrypoint-initdb.d/bron.sql"